    ...
    <ul dir='ltr' id='toc-list-root'>
      {#each toc as { depth, slug, title }}
        <li class='flex flex-col' id={`toc-item-${slug}`}>
          ...
          <span
            class="border-transparent transition-all cursor-pointer border-l-4 hover:border-primary hover:bg-base-content hover:bg-opacity-10 active:bg-primary active:text-primary-content active:font-bold pr-4 {depth <= 2 ? 'py-3' : 'py-2'}"
            class:pl-4={depth <= 2}
            class:pl-8={depth === 3}
            class:pl-12={depth === 4}
            class:pl-16={depth === 5}
